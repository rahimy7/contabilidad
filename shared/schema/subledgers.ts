...
export const arOpenItems = pgTable(
  "ar_open_items",
  {
    ...
    documentId: bigint("document_id", { mode: "number" }).references(() => fiscalDocuments.id, { onDelete: "cascade" }),
...
export const arApplications = pgTable(
  "ar_applications",
  {
    ...
    openItemId: bigint("open_item_id", { mode: "number" }).references(() => arOpenItems.id, { onDelete: "cascade" }).notNull(),
    amount: money("amount").notNull(),
  },
...
export const apApplications = pgTable(
  "ap_applications",
  {
    ...
    openItemId: bigint("open_item_id", { mode: "number" }).references(() => apOpenItems.id, { onDelete: "cascade" }).notNull(),
    amount: money("amount").notNull(),
  },
