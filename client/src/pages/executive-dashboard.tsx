    ...
    <div className="p-6 space-y-6">
      <div className="flex justify-between items-center">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-[20px] font-semibold tracking-tight">Panel Ejecutivo</h1>
        ...
        <Card>
          <CardHeader>
            <div className="flex justify-between items-center">
              ...
              <Link href="/customers"><span className="text-xs text-primary hover:underline">ver todos</span></Link>
            </div>
          </CardHeader>
          <CardContent>
                ...
                {d.topCustomers.map((c: any, i: number) => (
                  <div key={c.id} className="flex justify-between items-center p-2 rounded hover:bg-muted">
                    <div className="flex items-center gap-2 min-w-0">
                      <div className="w-6 h-6 rounded-full bg-accent dark:bg-accent text-primary flex items-center justify-center text-xs font-bold">
                        {i + 1}
                      </div>
        ...
        <Card>
          <CardHeader>
            <div className="flex justify-between items-center">
              ...
              <Link href="/product-management"><span className="text-xs text-primary hover:underline">ver todos</span></Link>
            </div>
          </CardHeader>
          <CardContent>
...
function AlertPill({ icon, label, value, severity, href }: {
  ...
  const cls = severity === "danger" ? "border-destructive bg-destructive/10 text-destructive dark:text-destructive"
    : severity === "warning" ? "border-warning bg-warning/15 text-warning dark:text-warning"
    : "border-primary bg-accent dark:bg-accent/20 text-primary dark:text-primary/70";
  return (
    <Link href={href}>
...
function SalesCard({ sales }: { sales: any }) {
  return (
    <Card>
      <CardHeader>
        <CardTitle className="text-base flex items-center gap-2">
          <DollarSign className="w-4 h-4 text-success" /> Ventas
        </CardTitle>
      </CardHeader>
          ...
          <div className="flex items-baseline gap-2">
            ...
            {sales.thisMonth.changePct != null && (
              <span className={`text-sm font-medium ${
                sales.thisMonth.direction === "up" ? "text-success" :
                sales.thisMonth.direction === "down" ? "text-destructive" : "text-muted-foreground"
              }`}>
...
function CashCard({ cash }: { cash: any }) {
  return (
    <Card>
      <CardHeader>
        <CardTitle className="text-base flex items-center gap-2">
          <Wallet className="w-4 h-4 text-primary" /> Posición de Caja
        </CardTitle>
      </CardHeader>
      <CardContent className="space-y-3">
        <div>
          <p className="text-xs text-muted-foreground">Posición neta</p>
          <p className={`text-3xl font-bold font-mono ${cash.netPosition >= 0 ? "text-primary" : "text-destructive"}`}>
        ...
        <div className="space-y-1 text-xs">
          ...
          <div className="flex justify-between">
            <span className="text-muted-foreground text-success">↗ Por cobrar (AR)</span>
            <span className="font-mono text-success">+RD$ {money(cash.arTotal)}</span>
          </div>
          <div className="flex justify-between">
            <span className="text-muted-foreground text-destructive">↘ Por pagar (AP)</span>
            <span className="font-mono text-destructive">−RD$ {money(cash.apTotal)}</span>
          </div>
        </div>
      </CardContent>
    </Card>
...
function PurchasesCard({ purchases }: { purchases: any }) {
  return (
    <Card>
      <CardHeader>
        <CardTitle className="text-base flex items-center gap-2">
          <Receipt className="w-4 h-4 text-primary" /> Compras · ITBIS
        </CardTitle>
      </CardHeader>
      <CardContent className="space-y-3">
        <div>
          <div className="flex items-baseline gap-2">
            ...
            {purchases.thisMonth.changePct != null && (
              <span className={`text-xs font-medium ${purchases.thisMonth.direction === "up" ? "text-destructive" : "text-success"}`}>
        ...
        <div className="pt-2 border-t space-y-1 text-xs">
          <div className="flex justify-between">
            ...
            <span className="font-mono text-success">+{money(purchases.itbisCollected)}</span>
          </div>
          <div className="flex justify-between">
            <span className="text-muted-foreground">ITBIS pagado (compras)</span>
            <span className="font-mono text-destructive">−{money(purchases.itbisPaid)}</span>
          </div>
          <div className="flex justify-between pt-1 border-t font-semibold">
            <span>ITBIS a pagar DGII</span>
            <span className={`font-mono ${purchases.itbisNet > 0 ? "text-destructive" : "text-success"}`}>
    ...
    <Card>
      ...
      <CardContent className="space-y-3">
        <div className="space-y-2 text-sm">
          <AgingRow label="Vigente (0 días)" value={data.current} pct={currentPct} color="bg-success" />
          <AgingRow label="1-30 días" value={data.days30_60} pct={d3060Pct} color="bg-warning" />
          <AgingRow label="31-60 días" value={data.days60_90} pct={d6090Pct} color="bg-warning" />
          <AgingRow label="60+ días" value={data.over90} pct={over90Pct} color="bg-destructive" />
        </div>
